// Grabs JSON configuration for the module from the HubL data.

const moduleConfig = JSON.parse(document.querySelector('script[data-search_results-config]').textContent);
...
var hsResultsPage = function (_resultsClass) {
  function buildResultsPage(_instance) {
    ...
    function addResult(title, url, description, featuredImage) {
      ...
      var mainLink = newResult.querySelector('.hs-search-results__link');
      if (mainLink) mainLink.href = url;

      // Set the bottom "Read now" link URL
      var bottomLink = newResult.querySelector('.hs-search-results__link-bottom');
      if (bottomLink) bottomLink.href = url;
      ...
      // Remove the date element since we don't have date info from search results
      var dateElement = newResult.querySelector('.hs-search-results__date');
      if (dateElement) {
        dateElement.remove();
      }
    ...
    function setSearchBarDefault(searchedTerm) {
      ...
      Array.prototype.forEach.call(searchBars, function (el) {
        el.value = decodeHTMLEntities(searchedTerm);
      });
    }
